...
class EventHandler(commands.Cog):
    ...
    async def on_message(self, ctx: discord.Message):
        ...
        if int(guild.channel_id) != ctx.channel.id:
            return

        current_time = int(time.time())
        last_message = await MessagesDao.get_last_messages_by_guild(int(guild.guild_id))

        if last_message is None:
            LOGGER.info(f"{ctx.id} {guild.guild_id} {current_time} {ctx.author.id}")
            await MessagesDao.add_message(ctx.id, int(guild.guild_id), current_time, ctx.author.id)
            LOGGER.info(f"Added message {ctx.id} to {ctx.guild.name} at {current_time}")
            return
        if int(last_message.author_id) == ctx.author.id:
        ...
        if int(last_message.timestamp) + int(guild.delay_second) <= current_time:
            last_message_author = await UserDao.get_user(
                int(last_message.author_id), int(guild.guild_id)
            )

            if last_message_author is None:
                # First point for the user
                await UserDao.add_user(int(last_message.author_id), int(guild.guild_id), 1)
            ...
            member = await ctx.guild.fetch_member(int(last_message.author_id))
            embed = discord.Embed(
                title="Point Awarded!",
                description=(
                    f"{member.mention} "
        ...
        await MessagesDao.delete_message(int(last_message.message_id))
